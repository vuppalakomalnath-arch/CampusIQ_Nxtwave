...
export default function ChatDetailPage() {
  ...
  const { conversations, loadConversations, loadConversation, startNewChat, currentConversationId, deleteConversation } =
    useChatStore();
          ...
          <aside className="hidden lg:flex w-72 flex-col rounded-2xl border border-slate-800 bg-slate-900/60 p-4 backdrop-blur-xl">
            ...
            <div className="flex-1 overflow-y-auto space-y-1.5 pr-1">
              {conversations.length === 0 ? (
                <p className="text-[11px] text-slate-500 text-center py-6">No chat history</p>
              ) : (
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => {
                        ...
                        if (id === conv._id) router.push('/chat');
                      }}
